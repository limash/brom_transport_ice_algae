...
contains
    ...
    subroutine init_netcdf(self, fn, first_lvl, last_lvl, model)
        ...
        do ip = 1, size(model%state_variables)
            ...
            call check_err(nf90_def_var(self%nc_id, model%state_variables(ip)%path(ilast+1:),&
                           NF90_REAL, dim_ids, self%parameter_id(ip)))
            call check_err(set_attributes(ncid=self%nc_id, id=self%parameter_id(ip),&
        ...
        do ip = 1, size(model%diagnostic_variables)
            if (model%diagnostic_variables(ip)%save) then
                ...
                call check_err(nf90_def_var(self%nc_id, model%diagnostic_variables(ip)%path(ilast+1:),&
                               NF90_REAL, dim_ids, self%parameter_id_diag(ip)))
                call check_err(set_attributes(ncid=self%nc_id, id=self%parameter_id_diag(ip),&
    ...
    subroutine save_netcdf_algae(self, ice_l, first_lvl, last_lvl, julianday, ice)

        use ice_algae_lib

        class(netcdf_algae_o):: self
        integer, intent(in)  :: first_lvl, last_lvl, julianday
        real(rk), intent(in) :: ice
        type(ice_layer), pointer, dimension(:), intent(in)  :: ice_l
        !internal parameters
        real(rk), dimension(last_lvl):: z, z_viz
        real(rk), dimension(last_lvl):: par_z
        real(rk), dimension(last_lvl):: bulk_temperature, bulk_salinity, bulk_density
        real(rk), dimension(last_lvl):: brine_temperature, brine_salinity, brine_density
        real(rk), dimension(last_lvl):: a_carbon, a_nitrogen, a_phosphorus
        real(rk), dimension(last_lvl):: nh4, no2, no3, po4
        real(rk), dimension(last_lvl):: brine_relative_volume
        ...
        do i = last_lvl, 1, -1
            call ice_l(i)%get_algae(z(i), par_z(i), bulk_temperature(i),&
                ...
                a_phosphorus(i), nh4(i), no2(i), no3(i), po4(i), brine_relative_volume(i))
        end do
        ...
        !z_viz is a number of the ice layer
        ...
        if (self%nc_id .ne. -1) then
            ...
            call check_err(nf90_put_var(self%nc_id, self%brine_temperature_id,&
                                        brine_temperature(first_lvl:last_lvl), start, edges))
